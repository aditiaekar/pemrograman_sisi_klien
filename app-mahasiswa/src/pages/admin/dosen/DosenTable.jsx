...
export default function DosenTable({ rows, loading, openEditModal, onDelete }) {
      ...
      <tbody>
        {rows.map((r, i) => (
          <tr key={r.id ?? i} className={i % 2 ? "bg-gray-100" : "bg-white"}>
            ...
            <td className="py-2 px-4">{r.nidn}</td>
        ...
        {rows.length === 0 && (
          <tr>
            <td className="py-4 px-4 text-center text-gray-500" colSpan={5}>Belum ada data.</td>
          </tr>
        )}
      </tbody>
    </table>
